<?php 
    ...
    class PersonaModel extends Modelo
    {
        ...
        {
            $personas = $this->_db->query ("SELECT p.id,p.nombre,r.nombre as rol,c.nombre as comuna FROM personas p 
            INNER JOIN roles r ON p.rol_id = r.id  INNER JOIN comunas c on p.comuna_id = c.id ORDER BY p.nombre");
        ...
        public function getPersonaId($id)
        {
            ...
            $persona = $this->_db->prepare("SELECT p.id, p.nombre, p.rut, p.email,p.direccion, p.fecha_nac, p.telefono, p.rol_id, p.comuna_id, p.created_at, p.updated_at, r.nombre as rol,c.nombre as comuna FROM personas p 
            INNER JOIN roles r ON p.rol_id = r.id  
            INNER JOIN comunas c ON p.comuna_id = c.id WHERE p.id = ?");
        ...
        //metodo que consulta por el registra de una persona por su email
        ...
        public function setPersona($nombre,$rut,$email,$direccion,$fecha_nac,$telefono,$rol,$comuna)
        {
            ...
            $persona = $this->_db->prepare("INSERT INTO personas(nombre,rut,email,direccion,fecha_nac,telefono,rol_id,comuna_id,created_at,updated_at) 
            VALUES(?,?,?,?,?,?,?,?,now(),now())");
