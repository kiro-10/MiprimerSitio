...
class comunaModel extends Modelo
{
    ...
    {
        $comunas = $this->_db->query("SELECT c.id,c.nombre,r.nombre As region FROM comunas c INNER JOIN regiones r 
        ON c.region_id =r.id ORDER BY c.nombre");
    ...
    public function getComunaId($id)
    {
        ...
        $comuna = $this->_db->prepare("SELECT c.id,c.nombre,c.region_id,c.created_at,c.updated_at,
        r.nombre as region FROM comunas c INNER JOIN regiones r ON c.region_id = r.id WHERE c.id =?");
    ...
    public function updateComuna($id, $nombre,$region)
    {
        ...
        $comuna = $this->_db->prepare("UPDATE comunas SET nombre =  ?, region_id = ?, updated_at = now() 
        WHERE id =  ?");
